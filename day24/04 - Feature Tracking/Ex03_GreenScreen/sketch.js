...
function preload() {
  // load in our replacement image
  replacementImage = loadImage('images/egypt.jpg');
}
  ...
  if (capture.pixels.length > 0 && compareFrame.pixels.length > 0) {
    ...
    for (var i = 0; i < capture.pixels.length; i += 4) {
      ...
      if (dist(capture.pixels[i], capture.pixels[i+1], capture.pixels[i+2], compareFrame.pixels[i], compareFrame.pixels[i+1], compareFrame.pixels[i+2]) < threshold) {
        // make this pixel the same as the one from our replacementImage -
        // it is very similar to the one in the compare frame
        mergedFrame.pixels[i] = replacementImage.pixels[i];
        mergedFrame.pixels[i+1] = replacementImage.pixels[i+1];
        mergedFrame.pixels[i+2] = replacementImage.pixels[i+2];
